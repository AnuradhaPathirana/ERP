...
return new class extends Migration
{
    ...
    {
        // Drop FK on pivot table to comply with the no-hard-FK-across-modules rule
        Schema::table('inv_vehicle_drivers', function (Blueprint $table): void {
        ...
        Schema::create('inv_vehicle_masters', function (Blueprint $table): void {
            ...
            // ── Identity ──────────────────────────────────────────────────
            $table->string('vehicle_code', 20)->unique();              // internal fleet code
            $table->string('registration_number', 50)->unique();       // licence plate

            // ── Specifications ────────────────────────────────────────────
            $table->string('make', 100)->nullable();                   // e.g. Toyota
            $table->string('model', 100)->nullable();                  // e.g. Corolla
            ...
            $table->decimal('payload_capacity', 10, 2)->nullable();    // tonnes
